...
def plot_attn_stringdb_vs_aligned_scatter_rank(
    path: str,
    layer_attention,
    dataset,
    batch,
    ...
    """Scatter of StringDB vs Aligned attention *ranks* for common neighbors.

    Scores are ranked independently within each edge type before plotting.
    """
    context = build_plot_context(path, dataset, batch)
    ...
    def _get_seed_ranked_weights(key):
        edge_index, weights = layer_attention[key]
        weights = weights.mean(dim=-1).detach().cpu()
        edge_index = edge_index.detach().cpu()
        # Rank all edges within this edge type first
        ranked = _rank_values(weights)
        mask = edge_index[1] == 0
        neighbors = edge_index[0][mask]
        w = ranked[mask]
        return {int(n): float(v) for n, v in zip(neighbors, w)}

    w_stringdb = _get_seed_ranked_weights(stringdb_key)
    w_aligned = _get_seed_ranked_weights(aligned_key)
    ...
    plt.xlabel(f"Attention Rank ({stringdb_key[1]})")
    plt.ylabel(f"Attention Rank ({aligned_key[1]})")
    plt.title(f"Layer {layer_idx} Attention Rank Correlation\n{context.seed_label}")
    plt.legend([f"Pearson R = {corr:.3f}"], loc="upper left")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
...
def plot_shared_name_attention_boxplot_rank(
    path: str,
    layer_attention,
    dataset,
    batch,
    ...
    """Boxplot comparing alignment attention *ranks* for shared vs different protein names.

    Attention values are ranked within the aligned_with edge type before grouping.
    """
    if not dataset.uses_entryid:
        logger.info("Shared-name rank boxplot skipped: dataset does not use EntryIDs.")
        return
    ...
    # Rank all attention values within this edge type
    ranked_weights = _rank_values(attn_weights)

    # Edges pointing to seed (index 0), excluding self-loops
    mask = (edge_index[1] == 0) & (edge_index[0] != 0)
    src_indices = edge_index[0][mask].tolist()
    ranks = ranked_weights[mask].tolist()

    if not ranks:
        logger.info("No alignment edges to seed; skipping shared-name rank boxplot.")
        return
    ...
    shared, different = [], []
    for src_idx, r in zip(src_indices, ranks):
        neighbor_label = context.labels.get(src_idx, "")
        if "_" not in neighbor_label:
            continue
        neighbor_name = neighbor_label.rsplit("_", 1)[0]
    ...
    if not shared and not different:
        logger.info("No categorizable neighbors; skipping shared-name rank boxplot.")
        return
    ...
    records = [{"Attention Rank": r, "Group": "Shared name"} for r in shared]
    records += [{"Attention Rank": r, "Group": "Different name"} for r in different]
    df = pd.DataFrame(records)

    plt.figure(figsize=(6, 5))
    sns.boxplot(
        data=df, x="Group", y="Attention Rank",
    ...
    title = (
        f"Alignment Attention Rank by Protein Name (L{layer_idx})\n{context.seed_label}"
    )
    if p_val is not None:
        title += f"\nMann-Whitney U  p = {p_val:.4g}"
    plt.title(title)
    plt.ylabel("Attention rank")
    plt.tight_layout()
...
def plot_edge_attr_vs_attention_scatter_rank(
    path: str,
    layer_attention,
    dataset,
    batch,
    ...
    """Scatter of edge attribute value vs attention *rank* per protein-protein channel.

    Attention values are ranked within each edge type independently.
    """
    if not dataset.config["model"]["edge_attrs"]:
        logger.info("Edge attr vs attention rank scatter skipped: edge_attrs disabled.")
        return
    ...
    for key in target_keys:
        ...
        # Rank attention values within this edge type
        attn_ranked = _rank_values(attn_mean)
        ...
        for i in range(edge_index_attn.size(1)):
            ...
            if (s, d) in edge_to_attr:
                x_vals.append(edge_to_attr[(s, d)])
                y_vals.append(float(attn_ranked[i]))
        ...
        plt.ylabel("Attention Rank")
        plt.title(
            f"Edge Attr vs Attention Rank ({rel}, L{layer_idx})\n"
            f"{context.seed_label}  (R = {corr:.3f})"
        )
        plt.grid(True, alpha=0.2)
        plt.tight_layout()
